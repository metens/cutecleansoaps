...
/* =========================
   Session “identity” (no sign-in UI)
========================= */
...
if (!soapId) {
  if (elTitle) elTitle.textContent = "Soap not found";
  if (elReviews) elReviews.textContent = "Missing soap id in URL.";
} else {
  boot(soapId);
}

async function boot(soapId) {
  // Silent auth (no UI). This is required for your Firestore rules.
  ...
  if (elTitle) elTitle.textContent = soap?.name || prettifySoapId(soapId);
  ...
  if (elReviews) {
    elReviews.addEventListener("click", async (e) => {
      ...
      try {
        // Ensure we have an auth uid (matches your rules)
        await ensureAnonAuth();
  ...
  btnSubmit?.addEventListener("click", async () => {
    ...
    if (stars < 1 || stars > 5) {
      setMsg(elMsg, "Please pick a rating (1–5).");
      return;
    }
    if (text.length < 3) {
      setMsg(elMsg, "Please write a short review.");
      return;
    }
    ...
    try {
      ...
      setMsg(elMsg, "Thanks! Your review was posted.");
    } catch (err) {
      console.error("Review submit failed:", err);
      setMsg(elMsg, "Sorry — couldn’t post your review. Try again.");
    } finally {
